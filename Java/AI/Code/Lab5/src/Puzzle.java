...
public class Puzzle implements IPuzzleAlgo {
	...
	public int computeH1(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				tile++;
				if (currentState.getTile(i, j) != tile && tile != 9) {
					output++;
				}
			}
		}
	...
	public int computeH2(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				tile++;
				if (currentState.getTile(i, j) != tile && tile != 9) {
					result += (Math.abs((i - currentState.getLocation(tile)[0]))
							+ Math.abs((j - currentState.getLocation(tile)[1])));
				}
			}
		}
		return result;
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			int row = whiteTile[0] + 1;
			int col = whiteTile[1];
			if (row <= MAX_ROW - 1) {// Tile stands inside the map
				int tmp = currentState.getTile(row, col);
				result.updateTile(row, col, 0);
				result.updateTile(whiteTile[0], whiteTile[1], tmp);
				result.setH(computeH2(result));
			...
			int col = whiteTile[1] + 1;
			int row = whiteTile[0];
			if (col <= MAX_COL - 1) {// Tile stands inside the map
				int tmp = currentState.getTile(row, col);
				result.updateTile(row, col, 0);
				result.updateTile(whiteTile[0], whiteTile[1], tmp);
				result.setH(computeH2(result));
	...
	public Node executeGreedy(Puzzle model) {
		...
		while (frontier.size() != 0) {
			...
			if (model.computeH1(curNode) == 0) {
				explored.add(curNode);
				return curNode;
			}

			List<Node> children = model.getSuccessors(curNode);
		...
		while (frontier.size() != 0) {
			...
			for (Node child : children) {
				if (!frontier.contains(child) && !explored.contains(child)) {
					...
					child.setH(model.computeH1(child));
					frontier.add(child);
				} else if (frontier.contains(child) && (child.getG() > (curNode.getG() + 1))) {
					// update the pastcost
					child.setG(curNode.getG() +1);
					child.setH(model.computeH1(child));
				}
